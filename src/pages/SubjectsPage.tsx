...
const SubjectsPage = () => {
  ...
  const { data, isLoading, error } = useGetSubjectsQuery({ page, size: 10 });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error(err?.data?.message || 'Có lỗi xảy ra khi xóa môn học');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateSubject({ id: editingId, data: { code, name } }).unwrap();
        toast.success('Cập nhật môn học thành công!');
      } else {
        await createSubject({ code, name }).unwrap();
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Quản lý danh sách các môn học trong trường</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-gray-500 animate-pulse">Đang tải dữ liệu môn học...</div>
        ) : error ? (
                    ...
                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button onClick={() => setDeleteId(item.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors tooltip" title="Xóa">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900">{editingId ? 'Cập nhật môn học' : 'Thêm môn học mới'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-1">Mã môn học</label>
                <input required type="text" value={code} onChange={(e) => setCode(e.target.value)} className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 uppercase text-gray-900" placeholder="VD: PRM393" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-1">Tên môn học</label>
                <input required type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-gray-900" placeholder="VD: Lập trình di động" />
              </div>
              <div className="pt-4 flex justify-end space-x-3">
                ...
                <button type="submit" disabled={isCreating || isUpdating} className="px-4 py-2 bg-orange-600 text-white rounded-xl font-medium hover:bg-orange-700 transition-colors disabled:opacity-50">
                  {isCreating || isUpdating ? 'Đang lưu...' : (editingId ? 'Cập nhật' : 'Lưu môn học')}
                </button>
              </div>
            </form>
          </div>
      ...
      <ConfirmModal
        ...
        title="Xóa môn học"
        message="Bạn có chắc chắn muốn xóa môn học này không? Hành động này không thể hoàn tác."
